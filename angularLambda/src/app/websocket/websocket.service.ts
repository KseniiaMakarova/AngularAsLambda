...
export class WebsocketService implements IWebsocketService, OnDestroy {
  ...
  ngOnDestroy() {
    ...
    console.log("Unsubscribe from socket");
    this.statusSub.unsubscribe();
    let closed = this.websocketSub.closed;
  ...
  * */
  public send(event: string, data: any = {}): void {
    if (event && this.isConnected) {
      this.websocket$.next({ event: event, data }); //       this.websocket$.next(<any>JSON.stringify({ event, data }));
    } else {
